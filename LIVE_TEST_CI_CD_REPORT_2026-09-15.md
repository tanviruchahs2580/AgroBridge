...
### 5.1 History purge & re-verification (post-commit)
- `git filter-repo --path apps/api/backup-1787603760600.json --path apps/api/backup-1787603867609.json --path apps/api/backup-1787603968929.json --invert-paths` executed on `main`.
- Verified: `git log --all -- 'apps/api/backup-*.json'` returns nothing; gitleaks re-run over full history = 0 findings.
- Remote: `origin/main` force-pushed after the rewrite; stale dependabot refs (which also contained the blobs) deleted from origin and re-verified.
- Team note: anyone with a prior clone must re-clone (or `git fetch && git reset --hard origin/main`). GitHub still serves the old blobs to anyone holding the exact SHA until the support request + GC — request removal via GitHub Support for full erasure.
